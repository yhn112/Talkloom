...
let package = Package(
    ...
    platforms: [.macOS(.v15)],
    products: [
        .library(name: "TalkloomCore", targets: ["TalkloomCore"]),
    ...
    targets: [
        ...
        // The report shape and fixture validation every evaluator shares, so that two
        // engines cannot be measured two different ways and then compared.
        .target(
            name: "ASREvalSupport",
            dependencies: ["TalkloomCore"],
            swiftSettings: [.swiftLanguageMode(.v6), .treatAllWarnings(as: .error)]
        ),
        .executableTarget(
            name: "OpenRouterASREval",
            dependencies: ["TalkloomCore", "ASREvalSupport"],
            swiftSettings: [.swiftLanguageMode(.v6), .treatAllWarnings(as: .error)]
        ),
        .testTarget(
            name: "TalkloomCoreTests",
            dependencies: ["TalkloomCore"],
            swiftSettings: [.swiftLanguageMode(.v6), .treatAllWarnings(as: .error)]
        ),
    ]
