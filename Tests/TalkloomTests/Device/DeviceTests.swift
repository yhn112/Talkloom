...
///
/// The scheme sets `TALKLOOM_DEVICE_TESTS`, because xcodebuild does not forward the
...
@Suite(
    ...
    .timeLimit(.minutes(5)),
    .enabled(
        if: ProcessInfo.processInfo.environment["TALKLOOM_DEVICE_TESTS"] == "1",
        "set TALKLOOM_DEVICE_TESTS=1, or use the TalkloomDeviceTests scheme"
    )
)
struct DeviceTests {
