...
extension DeviceTests {
    ...
    struct Controller {
        ...
        @Test(
            ...
            .enabled(
                if: ProcessInfo.processInfo.environment["TALKLOOM_DEVICE_SWITCH_TEST"]
                    == "1",
                "run through the TalkloomDeviceSwitchTests scheme only when a person is ready to switch output devices"
            )
        )
        func outputDeviceSwitchPreservesBothLogicalTracks() async throws {
            let rootPath = try #require(
                ProcessInfo.processInfo.environment["TALKLOOM_DEVICE_SWITCH_ROOT"],
