...
macOS on Apple silicon, Xcode, and the tools in `Brewfile`. `scripts/doctor.sh` prints
what this machine actually has — versions, the selected developer directory, the SDK, the
signing identity, the Python tooling — and names where anything missing comes from. Run it
rather than trusting a version written down here; a version in prose is a copy that goes
stale without anyone noticing.

Setting up: `brew bundle`, then `uv sync` for the Python tooling in `.venv`, then
`scripts/make-signing-cert.sh` once.

What is not discoverable by asking the machine:
...
- Signing uses a local self-signed certificate, `Transcriber Dev`. It stays stable across
  builds, so granted permissions survive rebuilds — ad-hoc signatures do not.
- `swift-format` comes from the Xcode toolchain (`xcrun swift-format`), not from Homebrew:
  a second copy would be a second opinion about formatting.
- The Python tooling is evaluation and analysis only. Nothing in `.venv` ships in the app,
  which is Swift throughout.
