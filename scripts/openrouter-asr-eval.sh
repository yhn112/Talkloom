#!/usr/bin/env bash
# Run the synthetic ASR fixtures through the live OpenRouter boundary. The local key file,
# recordings, transcripts and reports remain git-ignored.
set -euo pipefail
...
run_id=${TRANSCRIBER_ASR_RUN_ID:-"openrouter-gemini-$(date -u +%Y%m%dT%H%M%SZ)"}
...
binary="$binary_directory/OpenRouterASREval"
